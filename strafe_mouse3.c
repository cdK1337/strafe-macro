...
static void set_key(BYTE press_scan, BYTE release_scan, int *press_flag, int *release_flag) {
    if (*release_flag) {
        if (*press_flag)
            switch_dir(press_scan, release_scan);
        else
...
int main(void) {
    printf("Auto-strafe latch by raw mouse input\n");
    printf("====================================\n\n");
    printf("Hold SPACE + move mouse left  -> A held (latch)\n");
    printf("Hold SPACE + move mouse right -> D held (latch)\n");
    printf("Key stays held until opposite move or SPACE release\n");
    printf("Close console to exit.\n\n");

    WNDCLASSW wc = {0};
    wc.lpfnWndProc = wnd_proc;
    wc.hInstance = GetModuleHandle(NULL);
    wc.lpszClassName = L"AutoStrafeWnd3";
    RegisterClassW(&wc);

    HWND hwnd = CreateWindowExW(0, wc.lpszClassName, L"AutoStrafe3",
